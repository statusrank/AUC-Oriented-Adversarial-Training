...
class BALANCEMNIST(torchvision.datasets.MNIST):
    cls_num = 10
    def __init__(self, root, imb_type='exp', imb_factor=0.01, rand_number=0, train=True,transform=None, target_transform=None,download=False, delta_path=None, model_name=None, method=None):
        super(BALANCEMNIST, self).__init__(root, train, transform, target_transform, download)
        ...
        if delta_path != None:
            ...
            if self.use_type and self.method=='adauc_ours':
    ...
    def __getitem__(self, index):
        ...
        if self.method=='adauc_ours' and self.use_type:
    ...
    def gen_imbalanced_data(self):
        new_data = []
        new_targets = []
        img_num_per_cls = self.img_num_list
        targets_np = np.array(self.targets, dtype=np.int64)
...
class IMBALANCEMNIST(torchvision.datasets.MNIST):
    cls_num = 10
    def __init__(self, root, imb_type='exp', imb_factor=0.01, rand_number=0, train=True,transform=None, target_transform=None,download=False, delta_path=None, model_name=None, use_type='train'):
    ...
    def __getitem__(self, index):
        ...
        if self.use_type == 'train':
            if not os.path.exists(os.path.join(self.delta_path, self.model_name, 'delta', str(int(index))+'_tensor.pt')):
                delta = torch.zeros_like(img)
                torch.save(delta, os.path.join(self.delta_path, self.model_name, 'delta', str(int(index))+'_tensor.pt'))
            else:
                delta = torch.load(os.path.join(self.delta_path, self.model_name, 'delta', str(int(index))+'_tensor.pt'))
